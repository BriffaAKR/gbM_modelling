...
filename_params_code = params.filename_params_code_Sim
print(filename_params_code)
...
filename_start = params.filename_start_Sim
...
file_name_output_Mcorrn_Sim = os.path.join('Output_batch_files/Mcorrn_Sim', filename_start + 'Mcorrn_Sim_'+filename_ending)
file_name_output_All_CGcorrn_Sim = os.path.join('Output_batch_files/Mcorrn_Sim', filename_start + 'All_CGcorrn_'+filename_ending)
...
LocusProperties_headings = ['gene_ID', 'segment_number','seed', 'N_CG', 'L_locus', 'CG_density',
                                           'Sim_mu_meth_frac', 'Sim_sigma_meth_frac', 
                                            'Sim_Mfrac_star_TimeMu_AccnMu',
                                            'Sim_Mfrac_star_TimeMu_AccnSigma',
                                            'Sim_Mfrac_star_TimeSigma_AccnMu',
                                            'Sim_Mfrac_star_TimeSigma_AccnSigma',
                                            'Sim_Mfrac_star_max_AccnMu',
                                            'Sim_Mfrac_star_max_AccnSigma',
                                            'Sim_Mfrac_star_min_AccnMu',
                                            'Sim_Mfrac_star_min_AccnSigma',
                                            'Sim_Mfrac_FirstTimeSteadyState_AccnMu',
                                            'Sim_Mfrac_FirstTimeSteadyState_AccnSigma',
                                            'Sim_Mfrac_MaxFluct_Magnitude_AccnMu',
                                            'Sim_Mfrac_MaxFluct_Magnitude_AccnSigma',
                                            'Sim_Mfrac_MaxFluct_Duration_AccnMu',
                                            'Sim_Mfrac_MaxFluct_Duration_AccnSigma',
                                            'Sim_FirstTimeChangedSite_AccnMu',
                                            'Sim_FirstTimeChangedSite_AccnSigma',
...
CodeProgress_headings = ['batch', 'N_gene', 'time_mins','All_pairs_total']
...
if i_batch == 0:
 ...
 #   create empty files for rest....
    open(file_name_output_Mcorrn_Sim,'w').close
    open(file_name_output_All_CGcorrn_Sim,'w').close
...
# for i_ in range(N_reps):
#     AllRepsMethFracs_headings.append('Dset1_meth_frac_'+str(i_))
# #    AllRepsXFracs_headings.append('Dset1_meth_frac_'+str(i_))
# # if i_batch = 0 create output files and exit code using sys.exit()
# if i_batch == 0:
#     # write out column heading
...
def get_CG_positions(N_, locus_start_, locus_end_):
# randomly assings CG-site possitions within locus for given number of sites

    # list to store CG site locations
    CG_positions_ = []

    while len(CG_positions_) < N_:
        temp_val = random.randint(locus_start_, locus_end_+1)
        if (temp_val not in CG_positions_ and (temp_val - 1) not in CG_positions_ and (temp_val + 1) not in CG_positions_):
            CG_positions_.append(temp_val)

    # sort list into ascending order
    CG_positions_.sort()
            
    return CG_positions_


# Eqbrm initial state
# uses random CG-site positions and All-U state
def create_initial_state_eqbrm_Col0like_regions(ID_, initial_state_choice_, P_choice_,
    ...
    CG_positions_temp_ = temp_df_.Start.tolist()

    N_CG_ = len(CG_positions_temp_)
    CG_positions_ = get_CG_positions(N_CG_, current_segment_start_, current_segment_end_)
    
    state_sim_input_ = np.full(N_CG_,0)
    state_expt_ = np.full(N_CG_,0) # enables use of same function sturcture

    # print()
    # print(CG_positions_temp_)
    # print(N_CG_, current_segment_end_, current_segment_start_, current_segment_end_ - current_segment_start_ + 1.)
    # print(CG_positions_)
    # print()
...
# calculate M_correlation_function for a single locus
# count distance between every pair of M sites 
# sort into bins. bin number = N_corrns_bins
def cal_Mcorrn_locus(N_CG_, state_in_, CG_positions_in_):
    ...
    return MM_dist_counts_, All_CG_dist_list_, All_pairs_total_



def single_gene_sim_MfracFlucts(N_gen_cc_, N_CG_, state_, CG_positions_,
    ...
    # make copy of input state
    state_temp = state_.copy()
    ...
    # analyse state
    n_u_, n_h_, n_m_ = count_state(state_temp,N_CG_)
    ...
    # state_matrix_ = np.array(state_temp)

    
    # calculate initial propensities
    coop_gains_propensities_array_, spont_gains_propensities_vector_, \
            coop_maint_propensities_array_, total_propensities_vector_ = \
                initial_propensities(N_CG_, state_temp, CG_positions_, spont_gain_params_, 
                                         coop_gain_params_, coop_maint_params_)
    ...
    while True:
        ...
        # state_matrix_ = np.vstack((state_matrix_,state_temp))            
        ...
        # update state
        if state_temp[change_index_] == 0:
            state_temp[change_index_] += 2
        elif state_temp[change_index_] == 2:
            state_temp[change_index_] -= 2
        else:
            print('ahdla')
        ...
        coop_gains_propensities_array_, spont_gains_propensities_vector_, \
            coop_maint_propensities_array_, total_propensities_vector_ = \
            update_propensities(N_CG_, state_temp, CG_positions_, spont_gain_params_, 
    ...
    # transients_meth_level_ = [state_temp ,  state_time_, state_matrix_]
    transients_meth_level_ = [state_temp]
...
# Function to find time-averaged statistics on fluctuations
...
# main code body!!
# loop to simulate using: single_gene_sim_MfracFlucts
...
Sim_Mcorrn_counts = np.zeros(N_corrns_bins)
...
#for i_gene in range(2000):
    ...
    for i_seg_current_gene in range(0,current_number_of_segments):
        ...
        Sim_locus_Mfrac_Final_pending = []
        Sim_locus_Mfrac_star_TimeMu_pending = []
        Sim_locus_Mfrac_star_TimeSigma_pending = []
        Sim_locus_Mfrac_star_max_pending = []
        Sim_locus_Mfrac_star_min_pending = []
        Sim_locus_Mfrac_FirstTimeSteadyState_pending = []
        Sim_locus_MaxFluct_Magnitude_pending = []
        Sim_locus_MaxFluct_Duration_pending = []
        Sim_locus_FirstTimeChangedSite_pending = []
        ...
        for i_reps in range(0,N_reps):
            ...
            # run simulation          
            sim_outputs_temp = single_gene_sim_MfracFlucts(N_gen_total_cc, N_CG, 
                                    state_gene, CG_positions_gene, spont_gain_params, coop_gain_params, coop_maint_params)



            # unpack simulation_outputs
            eqbrm_stats_locus_meth_level_temp = sim_outputs_temp[0]
            eqbrm_state_output_temp = sim_outputs_temp[1]

            Sim_Mfrac_times_output_temp = eqbrm_stats_locus_meth_level_temp[0]
            # Ufrac level not required. 
            Sim_Mfrac_levels_output_temp = eqbrm_stats_locus_meth_level_temp[2]

            Sim_state_output_temp = eqbrm_state_output_temp[0]


            # call function to find Time-averaged statistics
            M_Frac_star_mu_temp, M_Frac_star_simga_temp, M_Frac_star_max_temp, M_Frac_star_min_temp, FirstTimeSteadyState_temp, MaxFluct_Mag_temp, MaxFluct_Time_temp, FirstTimeChangedSite_temp = \
                    TimeSeries_stats(np.array(Sim_Mfrac_times_output_temp), np.array(Sim_Mfrac_levels_output_temp), TimeAverage_StartGen, N_gen_total, initial_state_choice)

            # call function to calcualte Mcorrn counts
            Sim_Mcorrn_counts_pending, All_CG_dist_list_pending, All_pairs_total_temp = cal_Mcorrn_locus(N_CG, Sim_state_output_temp, CG_positions_gene)
            Sim_Mcorrn_counts = np.add(Sim_Mcorrn_counts, Sim_Mcorrn_counts_pending)
            Data_All_CGcorrn_counts = np.add(Data_All_CGcorrn_counts,All_CG_dist_list_pending)
            All_pairs_total += All_pairs_total_temp


            # Extend the pending lists
            Sim_locus_Mfrac_Final_pending.append(Sim_Mfrac_levels_output_temp[-1])
            Sim_locus_Mfrac_star_TimeMu_pending.append(M_Frac_star_mu_temp)
            Sim_locus_Mfrac_star_TimeSigma_pending.append(M_Frac_star_simga_temp)
            Sim_locus_Mfrac_star_max_pending.append(M_Frac_star_max_temp)
            Sim_locus_Mfrac_star_min_pending.append(M_Frac_star_min_temp)
            Sim_locus_Mfrac_FirstTimeSteadyState_pending.append(FirstTimeSteadyState_temp)
            Sim_locus_MaxFluct_Magnitude_pending.append(MaxFluct_Mag_temp)
            Sim_locus_MaxFluct_Duration_pending.append(MaxFluct_Time_temp)
            Sim_locus_FirstTimeChangedSite_pending.append(FirstTimeChangedSite_temp)
            ...
            with open(file_name_output_SimulatedStates_+str(i_reps)+'_'+filename_ending,'a',newline='') as output_file:
               for i_ in range(len(CG_positions_gene)):
                   ...
                   if Sim_state_output_temp[i_] == 0:
                       site_state_temp = 'U'
                   elif Sim_state_output_temp[i_] == 2:
                       site_state_temp = 'M'
                   else:
        ...
        Sim_mu_methylation_fraction = np.mean(Sim_locus_Mfrac_Final_pending)
        Sim_sigma_methylation_fraction = np.std(Sim_locus_Mfrac_Final_pending)

        Sim_Mfrac_star_TimeMu_AccnMu = np.mean(Sim_locus_Mfrac_star_TimeMu_pending)
        Sim_Mfrac_star_TimeMu_AccnSigma = np.std(Sim_locus_Mfrac_star_TimeMu_pending)
        Sim_Mfrac_star_TimeSigma_AccnMu = np.mean(Sim_locus_Mfrac_star_TimeSigma_pending)
        Sim_Mfrac_star_TimeSigma_AccnSigma = np.std(Sim_locus_Mfrac_star_TimeSigma_pending)
        Sim_Mfrac_star_max_AccnMu = np.mean(Sim_locus_Mfrac_star_max_pending)
        Sim_Mfrac_star_max_AccnSigma = np.std(Sim_locus_Mfrac_star_max_pending)
        Sim_Mfrac_star_min_AccnMu = np.mean(Sim_locus_Mfrac_star_min_pending)
        Sim_Mfrac_star_min_AccnSigma = np.std(Sim_locus_Mfrac_star_min_pending)
        Sim_Mfrac_FirstTimeSteadyState_AccnMu = np.mean(Sim_locus_Mfrac_FirstTimeSteadyState_pending)
        Sim_Mfrac_FirstTimeSteadyState_AccnSigma = np.std(Sim_locus_Mfrac_FirstTimeSteadyState_pending)
        Sim_Mfrac_MaxFluct_Magnitude_AccnMu = np.mean(Sim_locus_MaxFluct_Magnitude_pending)
        Sim_Mfrac_MaxFluct_Magnitude_AccnSigma = np.std(Sim_locus_MaxFluct_Magnitude_pending)
        Sim_Mfrac_MaxFluct_Duration_AccnMu = np.mean(Sim_locus_MaxFluct_Duration_pending)
        Sim_Mfrac_MaxFluct_Duration_AccnSigma = np.std(Sim_locus_MaxFluct_Duration_pending)
        Sim_FirstTimeChangedSite_AccnMu = np.mean(Sim_locus_FirstTimeChangedSite_pending)
        Sim_FirstTimeChangedSite_AccnSigma = np.std(Sim_locus_FirstTimeChangedSite_pending)
        ...
        # AllRepsMethFracs_temp.extend(Sim_locus_mean_M_level_pending)
...
temp_output_list = Sim_Mcorrn_counts
temp_output_file = file_name_output_Mcorrn_Sim
with open(temp_output_file,'a',newline='') as output_file:
   line_writer = csv.writer(output_file,delimiter='\t')
   line_writer.writerow(temp_output_list)
...
temp_output_file = file_name_output_All_CGcorrn_Sim
with open(temp_output_file,'a',newline='') as output_file:
   line_writer = csv.writer(output_file,delimiter='\t')
   line_writer.writerow(temp_output_list)
...
with open(file_name_output_CodeProgress,'a',newline='') as output_file:
    line_writer = csv.writer(output_file,delimiter='\t')
    line_writer.writerow([i_batch, i_gene+1, (timer_end_total - timer_start_total)/60., All_pairs_total])
