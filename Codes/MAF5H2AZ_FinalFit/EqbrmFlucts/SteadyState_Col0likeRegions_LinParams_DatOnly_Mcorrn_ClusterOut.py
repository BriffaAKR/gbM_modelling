...
accessions_state_file = os.path.join( path_string, file_in_accessions_states )
...
filename_start = params.filename_start_Dat
...
required_accessions_filenames_df = pd.read_csv(accessions_codes_file,sep='\t')
required_accessions_filenames_list = required_accessions_filenames_df['Accession_code'].tolist()
...
# accessions_state_file

# create lazy reader object
data_type_dict = dict.fromkeys(required_accessions_filenames_list, 'str')
data_type_dict.update({'gene_ID':'str', 'Start':'int64'})
All_accessions_states_df = dd.read_csv(accessions_state_file,sep='\t',
                            usecols=['gene_ID','Start',*required_accessions_filenames_list],
                        dtype=data_type_dict)
# define filtering logic
All_accessions_states_df = All_accessions_states_df[All_accessions_states_df['gene_ID'].isin(input_gene_ID_list)  ] 
All_accessions_states_df = All_accessions_states_df.set_index('gene_ID')
# apply filtering logic and convert to pandas dataframe
All_accessions_states_df = All_accessions_states_df.compute()
N_ExptState_Dset1 = len(required_accessions_filenames_list)
print('Check accessions number', N_ExptState_Dset1, len(All_accessions_states_df.columns)-1)
#print(All_accessions_states_df.columns.tolist())
#print(All_accessions_states_df.iloc[0,:].tolist())
print()
...
# for i_ in range(N_ExptState_Dset1):
...
#for i_gene in range(2000):
    ...
    for i_seg_current_gene in range(0,current_number_of_segments):
        ...
        if N_CG < N_CG_min:
            ...
            temp_output_list = [current_gene_ID, N_CG, 'na', 'Col0']
        ...
        if N_CG_density < N_CG_density_min:
            invalidate_current_ID = 1
            temp_output_list = [current_gene_ID, N_CG, N_CG_density, 'Col0']
            temp_output_file = file_name_output_Skipped_IDs
            with open(temp_output_file,'a',newline='') as output_file:
                line_writer = csv.writer(output_file,delimiter='\t')
                line_writer.writerow(temp_output_list)
        ...
        # set current dataframe to first expt. state
            #print('Current Accession', i_expts, required_accessions_filenames_list[i_expts])
            #print(All_accessions_states_df.iloc[:4,:4])
            #print()
            current_CG_site_data_df = All_accessions_states_df[['Start',required_accessions_filenames_list[i_expts]]]
            current_CG_site_data_df = current_CG_site_data_df.rename({required_accessions_filenames_list[i_expts]: 'Status'}, axis='columns')
            ...
            if N_CG_temp != N_CG:
                invalidate_current_ID = 1
                temp_output_list = [current_gene_ID, N_CG_temp, 'na', i_expts]
                temp_output_file = file_name_output_Skipped_IDs
                with open(temp_output_file,'a',newline='') as output_file:
                    line_writer = csv.writer(output_file,delimiter='\t')
                    line_writer.writerow(temp_output_list)
            ...
            if N_CG_density_temp != N_CG_density:
                invalidate_current_ID = 1
                temp_output_list = [current_gene_ID, N_CG_temp, N_CG_density_temp, i_expts]
                temp_output_file = file_name_output_Skipped_IDs
                with open(temp_output_file,'a',newline='') as output_file:
                    line_writer = csv.writer(output_file,delimiter='\t')
                    line_writer.writerow(temp_output_list)
        ...
        # AllRepsMethFracs_temp.extend(Dset1_locus_mean_M_level_pending)

        # AllRepsXFracs_temp = [current_gene_ID, current_segment_no]
        # AllRepsXFracs_temp.extend(Dset1_locus_mean_X_level_pending)
...
temp_output_list = Data_Mcorrn_counts
temp_output_file = file_name_output_Mcorrn_Dat
with open(temp_output_file,'a',newline='') as output_file:
   line_writer = csv.writer(output_file,delimiter='\t')
   line_writer.writerow(temp_output_list)
