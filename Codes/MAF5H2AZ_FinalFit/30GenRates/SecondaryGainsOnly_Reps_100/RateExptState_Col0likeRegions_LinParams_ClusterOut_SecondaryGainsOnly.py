...
CodeProgress_headings = ['batch', 'N_gene', 'time_mins', 'Sim_total_gains', 'Sim_total_losses', 'Sim_total_stayM', 'Sim_total_statyU']
...
def changed_sites_analysis(N_, state_initial_, state_final_, CG_positions_):
    ...
    return     gains_nn_dist_M_, gains_nn_dist_U_, losses_nn_dist_M_, losses_nn_dist_U_, \
                ...
                total_gains_, total_losses_, total_stayM_, total_stayU_
...
# Variables to count total changes
Sim_total_gains_pending = 0
Sim_total_losses_pending = 0
Sim_total_stayM_pending = 0
Sim_total_statyU_pending = 0
...
#for i_gene in range(2000):
    ...
    for i_seg_current_gene in range(0,current_number_of_segments):
        ...
        # modified to turn off primary gains manually here
        u_M_plus_PL = 0.0
        u_M_plus_PL_LR1 = u_scale_val*max(1.E-4*(u_m_val*N_CG_density  + u_c_val),1.E-4*u_cap_val)
        ...
        for i_reps in range(0,N_reps):
            ...
            gains_nn_dist_M_temp, gains_nn_dist_U_temp, losses_nn_dist_M_temp, losses_nn_dist_U_temp, \
                stayU_nn_dist_M_temp, stayU_nn_dist_U_temp, stayM_nn_dist_M_temp, stayM_nn_dist_U_temp, \
                total_gains_temp, total_losses_temp, total_stayM_temp, total_stayU_temp \
                 = changed_sites_analysis(N_CG, state_gene, eqbrm_state_output_temp, CG_positions_gene)
            ...
            Sim_total_gains_pending += total_gains_temp
            Sim_total_losses_pending += total_losses_temp
            Sim_total_stayM_pending += total_stayM_temp
            Sim_total_statyU_pending += total_stayU_temp
        ...
        #D3_UU_nearest_gaps_counts_pending, bins_dummy = np.histogram(D3_UU_nearest_gaps_pending, bins = N_corrns_bins,range=(-0.5,N_corrns_bins-0.5))
        #D3_MU_nearest_gaps_counts_pending, bins_dummy = np.histogram(D3_MU_nearest_gaps_pending, bins = N_corrns_bins,range=(-0.5,N_corrns_bins-0.5))
        #D3_MM_nearest_gaps_counts_pending, bins_dummy = np.histogram(D3_MM_nearest_gaps_pending, bins = N_corrns_bins,range=(-0.5,N_corrns_bins-0.5))
        #D3_XX_nearest_gaps_counts_pending, bins_dummy = np.histogram(D3_XX_nearest_gaps_pending, bins = N_corrns_bins,range=(-0.5,N_corrns_bins-0.5))
...
with open(file_name_output_CodeProgress,'a',newline='') as output_file:
    ...
    Sim_total_gains_pending, Sim_total_losses_pending, Sim_total_stayM_pending, Sim_total_statyU_pending])
