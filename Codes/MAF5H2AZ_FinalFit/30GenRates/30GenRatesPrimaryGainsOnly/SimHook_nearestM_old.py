...
print('Total Gains', np.sum(SimGains_nn_dist_M_array))
print('Total Losses', np.sum(SimLosses_nn_dist_M_array))
...
fig0, ax0 = plt.subplots(2,1,figsize=(20,20))
...
ax0[0].fill_between(x_vals[2:],MovAv_gains_All_Min, MovAv_gains_All_Max, color='lightgrey', alpha = 0.8, label='All range $\pm 1\sigma$')
ax0[0].fill_between(x_vals[2:],MovAv_gains_Schmitz_Min, MovAv_gains_Schmitz_Max, color='plum', alpha = 0.6, label='Schmitz range $\pm 1\sigma$')
ax0[0].plot(x_vals[2:], MovAv_gains_All_mean, linewidth=2, color='grey',alpha=0.8, label = 'All mean\nSmoothed n = %d bp' % N_window_gain_Jay)
ax0[0].plot(x_vals[2:], MovAv_gains_Schmitz_mean, linewidth=2, color='k', label = 'Schmitz mean\nSmoothed n = %d bp' % N_window_gain_Jay)
ax0[0].plot(x_vals[2:], MovAv_gains_Lizzie_conservative, linewidth=1, color='k',linestyle='--', label = 'Hollwey\nSmoothed n = %d bp' % N_window_gain_Lizzie)


# losses 
ax0[1].fill_between(x_vals[2:],MovAv_losses_All_Min, MovAv_losses_All_Max, color='lightgrey', alpha = 0.8, label='All range $\pm 1\sigma$')
ax0[1].fill_between(x_vals[2:],MovAv_losses_Schmitz_Min, MovAv_losses_Schmitz_Max, color='lightseagreen', alpha = 0.6, label='Schmitz range $\pm 1\sigma$')
ax0[1].plot(x_vals[2:], MovAv_losses_All_mean, linewidth=2, color='grey',alpha=0.8, label = 'All mean\nSmoothed n = %d bp' % N_window_loss_Jay)
ax0[1].plot(x_vals[2:], MovAv_losses_Schmitz_mean, linewidth=2, color='k', label = 'Schmitz mean\nSmoothed n = %d bp' % N_window_loss_Jay)
ax0[1].plot(x_vals[2:], MovAv_losses_Lizzie_conservative, linewidth=1, color='k',linestyle='--', label = 'Hollwey\nSmoothed n = %d bp' % N_window_loss_Lizzie)

#Model
ax0[0].plot(x_vals[2:], MovAv_SimGainRate_nn_dist_M,label='Simulated\nSmoothed n = %d bp' % N_window, linewidth=2, color='r')
ax0[1].plot(x_vals[2:], MovAv_SimLossRate_nn_dist_M,label='Simulated\nSmoothed n = %d bp' % N_window, linewidth=2, color='b')
...
axins_0.set_xlim(300,1200)
axins_0.set_ylim(0,0.5)
...
ax0[0].set_ylim(0,0.0005)
ax0[0].set_ylim(0,0.0003)
ax0[0].set_ylim(0,0.0005)
ax0[0].set_ylim(0,0.0003)

ax0[0].set_xlabel("Distance to nearest M site (bps)", fontsize=20)
ax0[0].set_ylabel("Gain rate per cell cycle", fontsize=20)
# ,loc=(0.18,0.40)
#ax0[0].legend(fontsize=16,ncol=4,facecolor='white', framealpha=0.6)
ax0[0].tick_params(axis='both', which='major', labelsize=16)

ax0[1].set_ylim(0,0.0003)
ax0[1].set_xlabel("Distance to nearest M site (bps)", fontsize=20)
ax0[1].set_ylabel("Loss rate per cell cylce", fontsize=20)
ax0[1].legend(fontsize=16,ncol=4,loc=2,facecolor='white', framealpha=0.6)
...
ax0[0].set_xlim(0,600)
ax0[1].set_xlim(0,600)
...
fig0, ax0 = plt.subplots(2,1,figsize=(8,12))
...
fig0.savefig( os.path.join(GraphFolder,filename_start +'SimRates_NearestM'+'.png'), bbox_inches = 'tight')
...
ax0.set_xlabel("Distance to nearest M site (bps)", fontsize=14)
ax0.set_ylabel("Gain rate\n per cell cycle"+y_scale_label, fontsize=14)
# ,loc=(0.18,0.40)
#ax0[0].legend(fontsize=16,ncol=2,facecolor='white', framealpha=0.6)
...
fig0.savefig( os.path.join(GraphFolder,filename_start +'SimGains_NearestM'+fig_label+'.png'), bbox_inches = 'tight')
