...
def TimeSeries_stats(time_vals_, M_frac_vals_, TimeAverage_StartGen_, N_gen_burn_in_, initial_state_choice_):
    ...
    if initial_state_choice_ == '100U':
        ...
        FirstTimeSteadyState_index_= np.argmax(M_frac_vals_ <= mu_)
        FirstTimeSteadyState_ = time_vals_[FirstTimeSteadyState_index_]
    elif mutant_label == 'cmt3_coop_':
        FirstTimeSteadyState_index_= np.argmax(M_frac_vals_ <= mu_)
        FirstTimeSteadyState_ = time_vals_[FirstTimeSteadyState_index_]
    elif mutant_label == 'ddcc_coop_':
        FirstTimeSteadyState_index_= np.argmax(M_frac_vals_ <= mu_)
        FirstTimeSteadyState_ = time_vals_[FirstTimeSteadyState_index_]
    elif mutant_label == 'ros1_coop_':
        FirstTimeSteadyState_index_= np.argmax(M_frac_vals_ >= mu_)
        FirstTimeSteadyState_ = time_vals_[FirstTimeSteadyState_index_]
    elif mutant_label == 'h2az_coop_':
        FirstTimeSteadyState_index_= np.argmax(M_frac_vals_ >= mu_)
        FirstTimeSteadyState_ = time_vals_[FirstTimeSteadyState_index_]
    else:
...
#for i_gene in range(2000):
    ...
    for i_seg_current_gene in range(0,current_number_of_segments):
        ...
        for i_reps in range(0,N_reps):
            ...
            Sim_Mcorrn_counts_pending = cal_Mcorrn_locus(N_CG, Sim_state_output_temp, CG_positions_gene)
        ...
        AllRepsMethFracs_temp.extend(Sim_locus_Mfrac_Final_pending)
