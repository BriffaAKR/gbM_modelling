...
import input_params_Cap as params # import my params from separate python file
...
def single_gene_sim_eqbrm_single_time_stats(N_CG_, state_in_, CG_positions_,
    ...
    # check for negative propensities
    # throw error if min_propensity is negative
    min_coop_gain_prop_ = np.amin(coop_gains_propensities_array_)
    min_spont_gain_prop_ = np.amin(spont_gains_propensities_vector_)
    min_coop_maint_prop_ = np.amin(coop_maint_propensities_array_)
    min_overal_prop_ = np.amin([min_coop_gain_prop_,min_spont_gain_prop_,min_coop_maint_prop_])
    if ( min_overal_prop_ < 0 ):
        sys.exit('Custom: Negative propensity detected!')
...
#for i_gene in range(2000):
    ...
    for i_seg_current_gene in range(0,current_number_of_segments):
        ...
        gamma0 = min(max(1.E-4*g_cap_val,g_m_val*N_CG_density + g_c_val),1.)
